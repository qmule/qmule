...
FileModel::FileModel(DirNode* root, QObject* parent /*= 0*/) : BaseModel(root, parent)
{
}
...
bool FileModel::hasChildren(const QModelIndex & parent /*= QModelIndex()*/) const
{
    bool res = false;
...
QModelIndex FileModel::index(const FileNode* node)
{
...
bool FileModel::setData ( const QModelIndex & index, const QVariant & value, int role/* = Qt::EditRole*/)
{
    bool res = false;
...
QModelIndex FileModel::node2index(const FileNode* node) const
{
    Q_ASSERT(node);
...
int FileModel::node2row(const FileNode* node) const
{
    int row = -1;
