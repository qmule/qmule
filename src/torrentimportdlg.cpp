...
void TorrentImportDlg::importTorrent()
{
  ...
  if (dlg.exec()) {
    ...
    Session::instance()->addTorrent(torrent_path);
...
{  
    restoreGeometry(Preferences().value("TorrentImportDlg/dimensions").toByteArray());
}

void TorrentImportDlg::saveSettings()
{  
    Preferences().setValue("TorrentImportDlg/dimensions", saveGeometry());
}
