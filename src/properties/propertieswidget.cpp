...
PropertiesWidget::PropertiesWidget(QWidget *parent, MainWindow* main_window, TransferListWidget *transferList):
  ...
  connect(transferList, SIGNAL(currentTorrentChanged(Transfer)), this, SLOT(loadTorrentInfos(Transfer)));
  connect(PropDelegate, SIGNAL(filteredFilesChanged()), this, SLOT(filteredFilesChanged()));
  connect(stackedProperties, SIGNAL(currentChanged(int)), this, SLOT(loadDynamicData()));
  connect(Session::instance(), SIGNAL(savePathChanged(Transfer)),
          this, SLOT(updateSavePath(Transfer)));
  connect(Session::instance(), SIGNAL(metadataReceived(Transfer)), this, SLOT(updateTorrentInfos(Transfer)));
...
Transfer PropertiesWidget::getCurrentTorrent() const {
  return h;
}

void PropertiesWidget::updateSavePath(const Transfer& _h) {
  if (h.is_valid() && h == _h) {
    QString p;
...
void PropertiesWidget::updateTorrentInfos(const Transfer& _h) {
  if (h.is_valid() && h == _h) {
    loadTorrentInfos(h);
  }
}

void PropertiesWidget::loadTorrentInfos(const Transfer& _h) {
  clear();
  h = _h;
  if (!h.is_valid()) {
    clear();
...
void PropertiesWidget::askWebSeed() {
  ...
  if (!listWebSeeds->findItems(url_seed, Qt::MatchFixedString).empty()) {
    QMessageBox::warning(this, tr("qMule"),
...
void PropertiesWidget::on_changeSavePathButton_clicked() {
  ...
  if (!new_path.isEmpty()) {
    ...
    if (!Session::instance()->useTemporaryFolder() || h.is_seed()) {
