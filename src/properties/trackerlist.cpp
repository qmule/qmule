...
void TrackerList::loadStickyItems(const Transfer& h) {
  ...
  if (Session::instance()->isDHTEnabled() && h.has_metadata() && !h.priv()) {
  ...
  if (Session::instance()->isPexEnabled())
  ...
  if (Session::instance()->isLSDEnabled())
...
void TrackerList::loadTrackers() {
  ...
  QHash<QString, TrackerInfos> trackers_data = Session::instance()->getTrackersInfo(h.hash());
