...
void PeerListWidget::updatePeerHostNameResolutionState()
{
  if (Preferences().resolvePeerHostNames()) {
    if (!m_resolver) {
      ...
      connect(m_resolver, SIGNAL(ip_resolved(QString,QString)), this, SLOT(handleResolved(QString,QString)));
      loadPeers(true);
    }
  } else {
    if (m_resolver)
...
void PeerListWidget::loadPeers(bool force_hostname_resolution) 
{
    ...
    for(transferIt = transfers.begin(); transferIt != transfers.end(); transferIt++)
    {
        ...
        for(itr = peers.begin(); itr != peers.end(); itr++) 
        {
            ...
            QString peer_key = torrent_name + ":" + peer_ip;
            if(ec) 
                continue;
            ...
            {
                // Add new peer
                m_peerItems[peer_key] = addPeer(peer_ip, peer, torrent_name);
