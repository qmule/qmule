...
class trackerLogin : public QDialog, private Ui::authentication{
  ...
  public:
    trackerLogin(QWidget *parent, Transfer h): QDialog(parent), h(h) {
      setupUi(this);
      setAttribute(Qt::WA_DeleteOnClose);
      login_logo->setPixmap(QPixmap(QString::fromUtf8(":/Icons/oxygen/encrypted.png")));
      tracker_url->setText(h.current_tracker());
      connect(this, SIGNAL(trackerLoginCancelled(QPair<Transfer,QString>)), 
              parent, SLOT(addUnauthenticatedTracker(QPair<Transfer,QString>)));
      show();
    }
  ...
  signals:
    void trackerLoginCancelled(QPair<Transfer,QString> tracker);
  ...
  public slots:
    ...
    void on_cancelButton_clicked() {
      ...
      emit trackerLoginCancelled(QPair<Transfer,QString>(h, h.current_tracker()));
      close();
    }
};
