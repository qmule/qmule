...
public slots:
  void showRestartRequired() {
    ...
    Session::instance()->addConsoleMessage(tr("qBittorrent was just updated and needs to be restarted for the changes to be effective."), "red");
  }
  ...
  void refreshStatusBar() {
    ...
    const SessionStatus sessionStatus = Session::instance()->getSessionStatus();
    if (!Session::instance()->isListening()) {
    ...
    if (Session::instance()->isDHTEnabled()) {
  ...
  void toggleAlternativeSpeeds() {
    Session::instance()->useAlternativeSpeedsLimit(!Preferences().isAltBandwidthEnabled());
  }
  ...
  void capDownloadSpeed() {
    ...
    if (ok) {
      ...
      if (new_limit <= 0) {
        ...
        Session::instance()->setDownloadRateLimit(-1);
        if (!alt)
  ...
  void capUploadSpeed() {
    bool ok = false;
#if LIBTORRENT_VERSION_MINOR > 15
    int cur_limit = Session::instance()->get_torrent_session()->getSession()->settings().upload_rate_limit;
#else
    ...
    if (ok) {
      ...
      if (new_limit <= 0) {
        ...
        Session::instance()->setUploadRateLimit(-1);
        if (!alt)
