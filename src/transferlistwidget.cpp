...
TransferListWidget::TransferListWidget(QWidget *parent, MainWindow *main_window, Session *_BTSession):
...
void TransferListWidget::setMaxRatioSelectedTorrents() {
  ...
  UpDownRatioDlg dlg(useGlobalValue, currentMaxRatio, Session::MAX_RATIO, this);
...
void TransferListWidget::displayListMenu(const QPoint&) {
  ...
  // don't show menu when no selection
...
void TransferListWidget::currentChanged(const QModelIndex& current, const QModelIndex&) {
  ...
  if (current.isValid()) {
    ...
    h = BTSession->getTransfer(getHashFromRow(row));
    // Scroll Fix
    scrollTo(current);
  }
