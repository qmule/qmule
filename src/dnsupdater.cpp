...
void DNSUpdater::processIPUpdateReply(const QString &reply)
{
  ...
  if (code == "good" || code == "nochg") {
    Session::instance()->addConsoleMessage(tr("Your dynamic DNS was successfully updated."), "green");
    return;
  }
  if (code == "911" || code == "dnserr") {
    Session::instance()->addConsoleMessage(tr("Dynamic DNS error: The service is temporarily unavailable, it will be retried in 30 minutes."),
                                              "red");
    m_lastIP.clear();
  ...
  m_lastIP.clear();
  if (code == "nohost") {
    Session::instance()->addConsoleMessage(tr("Dynamic DNS error: hostname supplied does not exist under specified account."),
                                              "red");
    m_state = INVALID_CREDS;
    return;
  }
  if (code == "badauth") {
    Session::instance()->addConsoleMessage(tr("Dynamic DNS error: Invalid username/password."), "red");
    m_state = INVALID_CREDS;
    return;
  }
  if (code == "badagent") {
    Session::instance()->addConsoleMessage(tr("Dynamic DNS error: qBittorrent was blacklisted by the service, please report a bug at http://bugs.qbittorrent.org."),
                                              "red");
    m_state = FATAL;
    return;
  }
  if (code == "!donator") {
    Session::instance()->addConsoleMessage(tr("Dynamic DNS error: %1 was returned by the service, please report a bug at http://bugs.qbittorrent.org.").arg("!donator"),
                                              "red");
    m_state = FATAL;
    return;
  }
  if (code == "abuse") {
    Session::instance()->addConsoleMessage(tr("Dynamic DNS error: Your username was blocked due to abuse."),
                                              "red");
    m_state = FATAL;
    return;
  }
...
void DNSUpdater::updateCredentials()
{
  ...
  if (m_domain != pref.getDynDomainName()) {
    ...
    if (domain_regex.indexIn(m_domain) < 0) {
      Session::instance()->addConsoleMessage(tr("Dynamic DNS error: supplied domain name is invalid."),
                                                "red");
      m_lastIP.clear();
      m_ipCheckTimer.stop();
      m_state = INVALID_CREDS;
  ...
  if (m_username != pref.getDynDNSUsername()) {
    ...
    if (m_username.length() < 4) {
      Session::instance()->addConsoleMessage(tr("Dynamic DNS error: supplied username is too short."),
                                                "red");
      m_lastIP.clear();
      m_ipCheckTimer.stop();
      m_state = INVALID_CREDS;
  ...
  if (m_password != pref.getDynDNSPassword()) {
    ...
    if (m_password.length() < 4) {
      Session::instance()->addConsoleMessage(tr("Dynamic DNS error: supplied password is too short."),
                                                "red");
      m_lastIP.clear();
      m_ipCheckTimer.stop();
      m_state = INVALID_CREDS;
